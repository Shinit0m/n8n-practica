...
function validateForm() {
    ...
    // Validar correo del responsable
    const responsibleEmail = document.getElementById('responsibleEmail').value.trim();
    if (!responsibleEmail) {
        showError('responsibleEmailError', 'El correo del responsable es obligatorio.');
        isValid = false;
    } else if (!isValidEmail(responsibleEmail)) {
        showError('responsibleEmailError', 'Por favor ingrese un correo electrónico válido.');
        isValid = false;
    } else {
        hideError('responsibleEmailError');
    }

    // Validar canal de difusión
    const diffusionChannels = document.querySelectorAll('input[name="diffusionChannel"]:checked');
    if (diffusionChannels.length === 0) {
        showError('diffusionChannelError', 'Debe seleccionar al menos un canal de difusión.');
        isValid = false;
    } else {
        hideError('diffusionChannelError');
    }
...
function updatePreview() {
    ...
    const responsibleEmail = formData.get('responsibleEmail') || '';
    const diffusionChannels = formData.getAll('diffusionChannel') || [];
    ...
    if (responsibleEmail) {
        html += `<div><strong class="text-gray-700">Correo del responsable:</strong> <span class="text-gray-600">${escapeHtml(responsibleEmail)}</span></div>`;
    }
    
    if (diffusionChannels.length > 0) {
        const channelLabels = diffusionChannels.map(ch => channelNames[ch] || ch).join(', ');
        html += `<div><strong class="text-gray-700">Canal de difusión:</strong> <span class="text-gray-600">${escapeHtml(channelLabels)}</span></div>`;
    }
...
const N8N_WEBHOOK_URL = 'http://localhost:5678/webhook-test/formulario-1';
...
async function sendToN8NWebhook(activityData, files) {
    ...
    try {
        ...
        formDataToSend.append('responsibleEmail', activityData.responsibleEmail);
        // Agregar canales de difusión
        if (activityData.diffusionChannels && activityData.diffusionChannels.length > 0) {
            formDataToSend.append('diffusionChannelsCount', activityData.diffusionChannels.length.toString());
            activityData.diffusionChannels.forEach((channel, index) => {
                formDataToSend.append(`diffusionChannel_${index}`, channel);
            });
        }
...
function handleFormSubmit(event) {
    ...
    const activityData = {
        ...
        diffusionChannels: formData.getAll('diffusionChannel'),
